...
describe('Cancel Rent Payment', () => {
  ...
  it('should fail when rent is not found', async () => {   
    ...
    const response = await request(httpServer)
      .patch(`/rents/${id}/cancel-rent-payment`)
      .set('token', token);      
  ...
  it('should fail when cancelling for rent that is not yours', async () => {
    const response = await request(httpServer)
      .patch(`/rents/${rent._id}/cancel-rent-payment`)
      .set('token', token);

    expect(response.status).to.equal(HttpStatus.UNAUTHORIZED);      
    expect(response.body).to.deep.include({
      success: false,
      message: 'You are not authorized to cancel payment for this rent, it does not belong to you'
    });
  });
  ...
  it('should fail when rejecting rent that is in paying state', async () => {  
    ...
    expect(response.body).to.deep.include({
      success: false,
      message: 'Only rents at (paying) state can have the payment cancelled'
    });
  });
